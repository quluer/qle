...
class Detail_EweiShopV2Page extends AppMobilePage 
{
		...
		if( $needpay ) 
		{
			...
			else 
			{
				if( $paytype == "wechat" ) 
				{
					...
					$payinfo = array( "openid" => $_W["openid_wa"], "title" => $set["shop"]["name"] . ((empty($goods["type"]) ? "卡路里兑换" : "卡路里抽奖")) . " 单号:" . $log["logno"], "tid" => $log["logno"], "fee" => $goods["money"] * $num + $goods["dispatch"] );
	...
	public function lottery() 
	{
		...
		if( empty($log["paystatus"]) ) 
		{
			...
			{
				app_error(AppError::$ParamsError, "卡路里不足");
			}
		...
		if( 0 < $goods["money"] + $goods["dispatch"] && $log["paystatus"] < 1 ) 
		{
			if( $log["paytype"] == 0 ) 
			{
				m("member")->setCredit($openid, "credit2", 0 - ($goods["money"] + $goods["dispatch"]), "卡路里商城扣除余额度 " . $goods["money"]);
				$update["paystatus"] = 1;
			}
			if( $log["paytype"] == 1 ) 
			{
		...
		{
			m("member")->setCredit($openid, "credit1", 0 - $goods["credit"], "卡路里商城扣除卡路里 " . $goods["credit"]);
			$update["creditpay"] = 1;
		...
		if( $status == 2 && $update["creditpay"] == 1 ) 
		{
			...
			else 
			{
				if( $goods["goodstype"] == 2 ) 
				{
					...
					$data = array( "openid" => $openid, "logno" => $logno, "uniacid" => $_W["uniacid"], "type" => "0", "createtime" => TIMESTAMP, "status" => "1", "title" => $set["name"] . "卡路里商城兑换余额", "money" => $num, "remark" => $creditstr, "rechargetype" => "creditshop" );
