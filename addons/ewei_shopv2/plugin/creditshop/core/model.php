		...
		public function payResult($logno, $type, $total_fee, $app = false) 
		{
			...
			if (!(empty($log)) && ($log['paystatus'] < 1) && ($log['creditpay'] == 0)) 
			{
				...
				if ((0 < $goods['credit']) && empty($log['creditpay'])) 
				{
					...
					m('member')->setCredit($log['openid'], 'credit1', -$goods['credit'], '卡路里商城扣除卡路里 ' . $goods['credit']);
					$update['creditpay'] = 1;
				...
				if ($status == 2) 
				{
					...
					else if ($goods['goodstype'] == 2) 
					{
						...
						$data = array('openid' => $log['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => $set['name'] . '卡路里商城兑换余额', 'money' => $num, 'remark' => $creditstr, 'rechargetype' => 'creditshop');
		...
		public function sendMessage($id = 0) 
		{
			...
			if ($log['status'] == 2) 
			{
				...
				else if ($log['dispatchstatus'] != 1) 
				{
					...
					$msg = array( 'first' => array('value' => '恭喜您，商品兑换成功~', 'color' => '#4a5077'), 'keyword1' => array('title' => '奖品名称', 'value' => $goods['title'], 'color' => '#4a5077'), 'keyword2' => array('title' => '消耗卡路里', 'value' => $credits, 'color' => '#4a5077'), 'keyword3' => array('title' => '剩余卡路里', 'value' => $credit, 'color' => '#4a5077'), 'keyword4' => array('title' => '兑换时间', 'value' => date('Y-m-d H:i', time()), 'color' => '#4a5077'), 'remark' => array('value' => $remark, 'color' => '#4a5077') );
				...
				if (($log['dispatchstatus'] == 1) || ($log['dispatchstatus'] == -1)) 
				{
					...
					$msg = array( 'first' => array('value' => '卡路里商城商品兑换成功~', 'color' => '#4a5077'), 'keyword1' => array('title' => '商品信息', 'value' => $goods['title'], 'color' => '#4a5077'), 'keyword2' => array('title' => '订单金额', 'value' => $credits, 'color' => '#4a5077'), 'keyword3' => array('title' => '兑换时间', 'value' => date('Y-m-d H:i', $log['createtime']), 'color' => '#4a5077'), 'keyword4' => array('title' => '兑换码', 'value' => ($goods['isverify'] ? '已发放' : '物流配送'), 'color' => '#4a5077'), 'remark' => array('value' => $remark, 'color' => '#4a5077') );
