                        ...
                        if( $item["params"]["goodsdata"] == "0" ) 
                        {
                            if( !empty($item["data"]) && is_array($item["data"]) ) 
                            {
                                ...
                                if( !empty($goodsids) && is_array($goodsids) ) 
                                {
                                    ...
                                    if( $creditshop ) 
                                    {
                                        $goods = pdo_fetchall("select deduct,id, title, thumb, price as productprice, money as minprice, credit, total, showgroups, `type`, goodstype from " . tablename("ewei_shop_creditshop_goods") . " where id in( " . $newgoodsids . " ) and status=1 and deleted=0 and uniacid=:uniacid order by displayorder desc ", array( ":uniacid" => $_W["uniacid"] ));
                                    }
                                    else
                                    {
                                        $goods = pdo_fetchall("select deduct,id, title, subtitle, thumb, productprice, minprice, total,`type`,showlevels, showgroups,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice, bargain, merchid, sales, salesreal from " . tablename("ewei_shop_goods") . " where id in( " . $newgoodsids . " ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ", array( ":uniacid" => $_W["uniacid"] ));
                                    }
                        ...
                        if( $item["params"]["goodsdata"] == "0" ) 
                        {
                            if( !empty($item["data"]) && is_array($item["data"]) ) 
                            {
                                ...
                                if( !empty($goodsids) && is_array($goodsids) ) 
                                {
                                    ...
                                    if( $creditshop ) 
                                    {
                                        $goods = pdo_fetchall("select deduct,id, showlevels, showgroups from " . tablename("ewei_shop_creditshop_goods") . " where id in( " . $newgoodsids . " ) and status=1 and deleted=0 and uniacid=:uniacid order by displayorder desc ", array( ":uniacid" => $_W["uniacid"] ));
                                    }
                                    else
                                    {
                                        $goods = pdo_fetchall("select deduct,id, showlevels,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice,showgroups from " . tablename("ewei_shop_goods") . " where id in( " . $newgoodsids . " ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ", array( ":uniacid" => $_W["uniacid"] ));
                                    }
