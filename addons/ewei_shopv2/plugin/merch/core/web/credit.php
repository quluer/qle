...
class Credit_EweiShopV2Page extends PluginWebPage
{
    protected function main($status = 1)
    {
        ...
        if( $_GPC["export"] == "1" ) 
        {
            ...
            if( $iscreditmoney == 0 ) 
            {
                $columns[] = array( "title" => "兑换比例*卡路里/一元", "field" => "creditrate", "width" => 12 );
                $columns[] = array( "title" => "申请卡路里", "field" => "creditnum", "width" => 12 );
                $columns[] = array( "title" => "通过的卡路里", "field" => "passcreditnum", "width" => 12 );
                $columns[] = array( "title" => "卡路里兑换的金额", "field" => "passcreditmoney", "width" => 12 );
            }
