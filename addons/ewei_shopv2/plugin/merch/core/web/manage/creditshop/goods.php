...
class Goods_EweiShopV2Page extends MerchWebPage 
{
	...
	protected function post() 
	{
		...
		if( 0 < count($options) ) 
		{
			...
			else 
			{
				...
				$html .= "<th><div class=\"\"><div style=\"padding-bottom:10px;text-align:center;\">卡路里</div></div></th>";
		...
		if( $_W["ispost"] ) 
		{
			...
			if( !empty($id) ) 
			{
				...
				plog("creditshop.goods.edit", "编辑卡路里商城商品 ID: " . $id . " <br/>商品名称: " . $data["title"]);
			}
			else 
			{
				...
				$id = pdo_insertid();
				plog("creditshop.goods.add", "添加卡路里商城商品 ID: " . $id . "  <br/>商品名称: " . $data["title"]);
			}
			if( $data["hasoption"] ) 
			{
				$totalstocks = 0;
	...
	public function delete() 
	{
		...
		foreach( $items as $item ) 
		{
			...
			plog("creditshop.goods.delete", "删除卡路里商城商品 ID: " . $item["id"] . "  <br/>商品名称: " . $item["title"] . " ");
		}
	...
	public function status() 
	{
		...
		foreach( $items as $item ) 
		{
			...
			plog("creditshop.goods.edit", "修改卡路里商城商品 " . $item["id"] . " <br /> 状态: " . (($status == 0 ? "下架" : "上架")));
		}
		show_json(1, array( "url" => referer() ));
	}
	...
	public function property() 
	{
		...
		if( in_array($type, array( "istop", "isrecommand", "status", "displayorder", "title" )) ) 
		{
			...
			plog("creditshop.goods.edit", "修改卡路里商城商品" . $typestr . "状态   ID: " . $id . " " . $statusstr . " ");
		}
		show_json(1);
	}
