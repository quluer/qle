...
class Log_EweiShopV2Page extends PcMobileLoginPage 
{
	...
	public function Receivepacket() 
	{
		...
		$params = array('openid' => $openid, 'tid' => $log['logno'], 'send_name' => ($set['sendname'] ? $set['sendname'] : $_W['shopset']['shop']['name']), 'money' => $money, 'wishing' => ($set['sendname'] ? $set['wishing'] : '红包领到手抽筋，别人加班你加薪!'), 'act_name' => '卡路里兑换红包', 'remark' => '卡路里兑换红包');
	...
	public function paydispatch() 
	{
		...
		if ($paytype == 'wechat') 
		{
			...
			$params['title'] = $set['shop']['name'] . ((empty($goods['type']) ? '卡路里兑换' : '卡路里抽奖')) . ' 支付运费单号:' . $dispatchno;
		...
		else if ($paytype == 'alipay') 
		{
			...
			$params['user'] = $openid;
			$params['fee'] = $goods['dispatch'];
			$params['title'] = $set['shop']['name'] . '卡路里兑换运费支付' . ' 单号:' . $log['logno'];
