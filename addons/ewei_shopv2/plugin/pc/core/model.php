...
class PcModel extends PluginModel 
{
	...
	public function sendMessage($id = 0) 
	{
		...
		if ((0 < $goods['credit']) & (0 < $goods['money'])) 
		{
			$credits = $goods['credit'] . '卡路里+' . $goods['money'] . '元';
		}
		else if (0 < $goods['credit']) 
		{
			$credits = $goods['credit'] . '卡路里';
		}
		else if (0 < $goods['money']) 
		{
		...
		if ($log['status'] == 2) 
		{
			if (!(empty($type))) 
			{
				if ($log['dispatchstatus'] != 1) 
				{
					...
					$msg = array( 'first' => array('value' => '恭喜您，您中奖啦~', 'color' => '#4a5077'), 'keyword1' => array('title' => '活动', 'value' => '卡路里商城抽奖', 'color' => '#4a5077'), 'keyword2' => array('title' => '奖品', 'value' => $goods['title'], 'color' => '#4a5077'), 'remark' => array('value' => $remark, 'color' => '#4a5077') );
					if (!(empty($tm['award']))) 
					{
			...
			else if ($log['dispatchstatus'] != 1) 
			{
				...
				$msg = array( 'first' => array('value' => '恭喜您，商品兑换成功~', 'color' => '#4a5077'), 'keyword1' => array('title' => '奖品名称', 'value' => $goods['title'], 'color' => '#4a5077'), 'keyword2' => array('title' => '消耗卡路里', 'value' => $credits, 'color' => '#4a5077'), 'keyword3' => array('title' => '兑换时间', 'value' => date('Y-m-d', time()), 'color' => '#4a5077'), 'keyword4' => array('title' => '剩余卡路里', 'value' => $credit, 'color' => '#4a5077'), 'remark' => array('value' => $remark, 'color' => '#4a5077') );
			...
			if (($log['dispatchstatus'] == 1) || ($log['dispatchstatus'] == -1)) 
			{
				...
				$msg = array( 'first' => array('value' => '卡路里商城商品兑换成功~', 'color' => '#4a5077'), 'keyword1' => array('title' => '订单编号', 'value' => $log['logno'], 'color' => '#4a5077'), 'keyword2' => array('title' => '商品名称', 'value' => $goods['title'], 'color' => '#4a5077'), 'keyword3' => array('title' => '兑换时间', 'value' => date('Y-m-d', $log['createtime']), 'color' => '#4a5077'), 'keyword4' => array('title' => '兑换码', 'value' => ($goods['isverify'] ? '已发放' : '物流配送'), 'color' => '#4a5077'), 'remark' => array('value' => $remark, 'color' => '#4a5077') );
		...
		else if ($log['status'] == 3) 
		{
			...
			$msg = array( 'first' => array('value' => '您的卡路里兑换奖品已发货~', 'color' => '#4a5077'), 'keyword1' => array('title' => '订单金额', 'value' => '使用 ' . $credits, 'color' => '#4a5077'), 'keyword2' => array('title' => '商品详情', 'value' => $goods['title'], 'color' => '#4a5077'), 'keyword3' => array('title' => '收货信息', 'value' => $info, 'color' => '#4a5077'), 'remark' => array('value' => $remark, 'color' => '#4a5077') );
			if (!(empty($tm['send']))) 
			{
