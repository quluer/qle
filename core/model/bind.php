...
class Bind_EweiShopV2Model
{
	...
	public function merge($a = array(), $b = array())
	{
		...
		if (0 < $a['credit1']) {
			m('member')->setCredit($b['openid'], 'credit1', abs($a['credit1']), '全网通会员数据合并增加卡路里 +' . $a['credit1'] . $mergeinfo);
		}
	...
	public function sendCredit($member = array())
	{
		...
		if (!empty($data['bindmobile']) && 0 < intval($data['bindmobilecredit'])) {
			m('member')->setCredit($member['openid'], 'credit1', abs($data['bindmobilecredit']), '绑定手机号送卡路里 +' . $data['bindmobilecredit']);
		}
	}
	...
	public function mergeforuniacid($a = array(), $b = array())
	{
		...
		if (0 < $a['credit1']) {
			m('member')->setCredit($b['openid'], 'credit1', abs($a['credit1']), '数据迁移会员数据合并增加卡路里 +' . $a['credit1'] . $mergeinfo);
		}
